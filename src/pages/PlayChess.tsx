...
import { ArrowLeft, RotateCcw, Brain, Monitor, User, Undo2, History, Target, Search } from "lucide-react";
...
export function PlayChess() {
  ...
  const onDrop = ({ sourceSquare, targetSquare }: { sourceSquare: string; targetSquare: string | null }) => {
    if (!targetSquare || gameState?.turn !== "w" || gameState.status !== "playing" || isThinking) {
      return false;
    }
  ...
  const isCheck = chessGame.isCheck() || gameState.status === "checkmate";
